#!/bin/bash
set -e

# Create multiple databases
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  CREATE DATABASE auth_db;
  CREATE DATABASE user_db;
  CREATE DATABASE admin_db;
  CREATE DATABASE workout_db;
  CREATE DATABASE nutrition_db;
  CREATE DATABASE order_db;
  CREATE DATABASE payment_db;
  CREATE DATABASE partner_db;

  -- Grant permissions
  GRANT ALL PRIVILEGES ON DATABASE auth_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE user_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE admin_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE workout_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE nutrition_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE order_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE payment_db TO $POSTGRES_USER;
  GRANT ALL PRIVILEGES ON DATABASE partner_db TO $POSTGRES_USER;
EOSQL

# Run migrations for each database
echo "Running auth_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "auth_db" < /migrations/auth_db.sql

echo "Running user_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "user_db" < /migrations/user_db.sql

echo "Running admin_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "admin_db" < /migrations/admin_db.sql

echo "Running workout_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "workout_db" < /migrations/workout_db.sql

echo "Running nutrition_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "nutrition_db" < /migrations/nutrition_db.sql

echo "Running order_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "order_db" < /migrations/order_db.sql

echo "Running payment_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "payment_db" < /migrations/payment_db.sql

echo "Running partner_db migrations..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "partner_db" < /migrations/partner_db.sql

echo "Database initialization completed!"
