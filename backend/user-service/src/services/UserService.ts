...
const determineHost = (override?: string) =>
  normalizeHost(override) ||
  normalizeHost(process.env.ASSET_PUBLIC_HOST) ||
  "http://localhost:3000";

const absoluteUrl = (url?: string | null, hostOverride?: string) => {
  if (!url) return url;
  if (/^https?:\/\//i.test(url)) return url;
  const host = determineHost(hostOverride);
  if (url.startsWith("/")) {
    return `${host}${url}`;
  }
  return `${host}/${url}`;
...
export const buildPublicAvatarUrl = (
  url?: string | null,
  hostOverride?: string
...
export class UserService {
  ...
  static async updateUserProfile(
    ...
    // If user not found, try to get email from auth-service and create user
    if (!user) {
      // For Google users coming from auth-service, user might not exist yet
      // Check if user exists but not active (shouldn't happen, but handle it)
      ...
      // User doesn't exist at all - create a minimal user from payload
      // This handles Google users that exist in auth-service but not yet in user-service
      const newUser = new UserModel({
        user_id,
        name: payload.name || "User",
        email: `user-${user_id.slice(0, 8)}@local.dev`,
  ...
  static async createUserWithEmail(
    ...
    if (existingUser) {
      // If same user_id, return it
      if (existingUser.user_id === user_id) {
        return toUserResponse(existingUser);
      }
      // If different user_id but same email, also return it (handles Google login case)
      // where auth-service has the email but user-service needs to find/create the user
      return toUserResponse(existingUser);
    }
  ...
  static async getUserGoals(user_id: string) {
    ...
    return userGoals.map((userGoal) => ({
      ...
      target_completion_date:
        userGoal.target_completion_date?.toISOString() || null,
      actual_completion_date:
        userGoal.actual_completion_date?.toISOString() || null,
  ...
  static async assignGoalToUser(user_id: string, payload: AssignGoalPayload) {
    ...
    const userGoal = new UserGoalModel({
      ...
      target_completion_date: payload.target_completion_date
        ? new Date(payload.target_completion_date)
        : null,
      notes: payload.notes,
    ...
    return {
      ...
      target_completion_date:
        savedUserGoal.target_completion_date?.toISOString() || null,
      actual_completion_date:
        savedUserGoal.actual_completion_date?.toISOString() || null,
  ...
  static async updateUserGoalProgress(
    user_id: string,
    user_goal_id: string,
    payload: UpdateUserGoalPayload
  ) {
  ...
  static async getUserDashboard(user_id: string) {
    const [user, activeGoals, totalGoals] = await Promise.all([
      UserModel.findOne({ user_id, is_active: true })
        .select("-password")
        .lean(),
      UserGoalModel.countDocuments({
        user_id,
        status: { $in: ["Active", "Paused"] },
      }),
